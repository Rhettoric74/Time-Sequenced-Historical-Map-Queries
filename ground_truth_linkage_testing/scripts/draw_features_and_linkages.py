...
if __name__ == "__main__":
    ...
    for map_filename in random_map_sample:
        for weight in weights_list:
            #for descriptor, distance_function in descriptors_to_functions.items():
            mg = map_graph.MapGraph(map_filename)
            clustering_linkages.cluster_map_nodes(mg.nodes, weight)
            map_annotations = multiword_name_extraction.extract_map_data_from_all_annotations(map_filename)
            draw_features_and_linkages(map_filename, mg, "kmeans_" + "euclidean_" + str(weight[0]) + "-" + str(weight[1]) + "-" + str(weight[2]) + "_" + map_filename)