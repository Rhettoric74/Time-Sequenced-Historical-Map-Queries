...
if __name__ == "__main__":
    ...
    print(enough_multiword_phrases)
    #print(random_map_sample)
    old_weights_list = [[0.5 * i, 0.25 * j, 0.25 * k] for i in range(5) for j in range(5) for k in range(5)]
    weights_list = [[0.2 * i, 0.1 * j, 0.1 * k] for i in range(10) for j in range(10) for k in range(10)]
    weights_strings = ["weights_" + str(weights[0]) + "_" + str(weights[1]) + "_" + str(weights[2]) for weights in weights_list]

    maps_in_validation_set = os.listdir("icdar24-val-png/val_images")
    print(len(maps_in_validation_set))
    predicted_best_edge_cost_func = FeatureNode.EdgeCostFunction([1.51, 0.6, 0.36])
    results_dict = {"connecting_method": "MST", "distance_function_weights":[1.51, 0.6, 0.36], "map_results":[]}
    for map in enough_multiword_phrases:
        results_dict["map_results"].append(compare_linkages(map, "icdar24-train-png/annotations.json", LinkageMethod(prims_mst, predicted_best_edge_cost_func)))
    with open(os.getcwd().strip("/scripts") + "/mst_results/weighted_mst_results/all_maps_weights_1.51_0.60_0.36.json", "w") as f:
        json.dump(results_dict, f)
    print(get_stats_from_results_file(os.getcwd().strip("/scripts") + "/mst_results/weighted_mst_results/all_maps_weights_1.51_0.60_0.36.json"))
